...
function watchlistRowForAccount(tickerMeta, account) {
  ...
  return {
    ...
    fx_rate: fxRate,
    previous_fx_rate: Number.isFinite(Number(tickerMeta.previous_fx_rate)) ? Number(tickerMeta.previous_fx_rate) : fxRate,
    value: null,
    value_krw: null,
...
function syncDetailTabs() {
  ...
  // 성과 차트는 아이콘·이름('% 성과')이 툴바 타이틀과 겹친다. 헤드 줄 전체를 숨겨
  // 범례 위(헤드 margin)와 아래 여백이 어긋나지 않게 한다.
  document.querySelector("#chartView .chart-head")?.classList.toggle("hidden", performanceChartOpen);
...
function renderTable() {
  ...
  document.getElementById("holdings").innerHTML = rows.map(r => {
    ...
    <tr class="${tableRowClass(r)}">
      <td class="logo-cell">${companyProfileLogo(r)}</td>
      <td>
...
(window.__loaded = window.__loaded || new Set()).add("app-holdings");
