...
async function loadStatsForRows(rows) {
  ...
  statsLoadKey = key;
  document.getElementById("statsRows").innerHTML = `<tr><td colspan="22">통계 loading...</td></tr>`;
  statsInFlight = (async () => {
  ...
  try {
    ...
    document.getElementById("statsRows").innerHTML = `<tr><td colspan="22">${esc(err.message || String(err))}</td></tr>`;
  } finally {
    statsInFlight = null;
  }
}
