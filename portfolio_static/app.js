...
const DEFAULT_CHART_RANGE = "6m";   // 종목차트·성과차트 진입 시 기본 기간
...
document.addEventListener("click", event => {
  const profileBtn = event.target.closest?.("[data-company-profile]");
  if (profileBtn) {
    event.preventDefault();
    toggleCompanyProfile(profileBtn);
    return;
  }
  ...
  if (companyProfileAnchor && !event.target.closest?.("#companyProfilePopover")) closeCompanyProfile();
...
(window.__loaded = window.__loaded || new Set()).add("app");
