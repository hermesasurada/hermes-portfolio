...
const BETA_BENCHMARK_NOTE = "기준지수: 한국 종목은 KODEX 200TR(278530.KS), 그 외는 S&P 500";
const BETA_TOOLTIP = [
  "지수와 같이 움직인 정도 · 최근 252거래일(약 1년) 일간 수익률",
  "공분산(종목, 지수) ÷ 지수 분산",
  BETA_BENCHMARK_NOTE,
...
const BETA_ADJ_TOOLTIP = [
  "지수 대비 변동폭 배수 · 최근 252거래일(약 1년) 일간 수익률",
  "종목 표준편차 ÷ 지수 표준편차",
  BETA_BENCHMARK_NOTE + " — β와 같은 지수라 β = 상관계수 × β″, 항상 |β| ≤ β″",
  "방향이 달라 β가 낮아도 β″가 크면 그 자체로 많이 흔들리는 종목",
].join("\n");
