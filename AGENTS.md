...
- Periodic price-return displays above one year (3/5/10Y) use CAGR with one decimal and explicit CAGR labels across account/watchlist lists, stock details and comparisons. Convert via the shared frontend helper for display/sorting only; keep API cumulative returns and risk-score inputs unchanged. Periods of one year or less remain cumulative.
