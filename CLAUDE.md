...
- 관심목록 그룹 생성·이름/순서/삭제 및 종목 추가·삭제는 사이드바 **편집 → 완료** 모드에서만 노출. 모바일은 검색을 항상 보이고 섹터·통화·지수는 **필터** 버튼으로 접는다. 적용 중인 부가 필터는 접혀도 버튼에 표시. 편집 모드 전환 자체는 서버를 호출하지 않는다. 테스트: `node tests/test_design_controls.js`.
...
- **입출금 존재만으로 선택되는 full은 데이터 완전성 인증이 아니다.** 거래 이력이 불완전한 계좌는 입출금을 추가해도 과거 성과가 미검증 상태다. 입출금 입력 여부는 DB에서 확인하며, 미입력 계좌가 포함된 합산은 securities를 사용한다. 입출금 등록만으로 `history_start`나 보유·거래를 임의 변경하지 않는다.
  ...
  - id 2 Ray 연금저축: **2026-09-08 증권사 거래내역으로 원장 전면 대체**(사용자 지시). 2026-09-02 1건만 남기고 403건을 지운 뒤 510건 삽입 → 511건, **36종목 전부 잔고=거래합**, 미사용 현금 4,361만→839만, TWR(full) +73.0%→+82.1%. 원본 종목코드는 `A379800` 형식이라 A를 떼고 `.KS`를 붙여 매핑한다. 청산 완료된 8종목(069500·271060·305720·314250·372330·390400·394660·400580)은 이때 신규 등록하고 '투자종료' 관심그룹에 넣었다. 삭제분 백업은 스크래치패드 `ray_pension_deleted_backup.json`.
