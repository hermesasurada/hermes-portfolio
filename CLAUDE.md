...
- 상단 `입출금` → `app-cash-flows.js`의 조회 전용 팝업. 전체 계좌를 열·입출금이 있는 날짜를 행(최신순)으로 표시하며 같은 날·계좌·통화별 순액과 계좌별 누계를 집계한다. 다른 통화를 합산/현재 환율로 환산하지 않는다. 원장 항목(id 포함)은 셀 모델에 유지하므로 향후 입력·수정 UI를 붙일 때 재사용 가능. 테스트: `node tests/test_cash_flows_ui.js`.
