...
- 상단 `입출금` → `app-cash-flows.js`의 조회 전용 팝업. 계좌 스위치로 선택한 열과 해당 계좌에 입출금이 있는 날짜만 최신순 표시(전체 선택/해제 지원). 같은 날·계좌·통화별 순액과 계좌별 누계를 집계한 뒤 원화는 만원·소수점 1자리로 표시하며 숫자/날짜는 고정폭 글꼴 사용. 다른 통화를 합산/현재 환율로 환산하지 않는다. 원장 항목(id 포함)은 셀 모델에 유지하므로 향후 입력·수정 UI를 붙일 때 재사용 가능. 테스트: `node tests/test_cash_flows_ui.js`.
