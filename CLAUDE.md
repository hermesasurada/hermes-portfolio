...
- **입출금이 들어간 계좌는 id 1·4·6·9**(나머지는 미입력 → 합산은 securities, 단독 조회만 full).
  - id 1 Raymond 해외주식: 265건(2021-06-28~2026-08-21, 순 2.85억). `history_start='2021-06-28'`(첫 입금일) 고정 — 첫 입금이 공통 기준일(2021-10-20)보다 앞서서, 안 그러면 18건 3,771만원이 잘린다.
    ...
    - 특수 처리 셋: ① AAPL 120주는 2024-07-12 id 4로 **현물 증여** → 매도는 그대로 두고 같은 금액 **출금**(3,517만)을 넣어 대금이 안 들어온 것으로 맞췄다. ② SOLV 19주는 2024-04-01 **MMM 분할 배정(무상)** → 0원 매수로 수량만 넣는다(MMM 시세가 이미 조정돼 있어 자산 총량은 맞다). ③ ATVI 37주는 2023-10-17 상장폐지 현금정산 → 사용자 지시로 **$3,515 입금**으로 처리(야후가 상폐 종목이라 시세를 안 줘 평가가 불가능하다). 정산금이 외부 유입으로 잡혀 TWR에는 수익으로 안 잡힌다.
    - LLYX 마지막 매도를 확인받아 593→540주로 정정, **53종목 전부 잔고=거래합**. 현금잔고는 2024-07-12(증여 출금일)부터 음수, 현재 −1,906만.
  ...
  - id 4 Claire 해외주식: 98건(2021-11-09~2026-08-21, 순 2.79억). 2026-05-11 −3,306만·2026-06-01 −2,015만 출금은 id 6의 같은 날 입금과 정확히 맞물리는 계좌 간 이체다.
    ...
    - 거래이력 보강은 두 차례다: TQQQ·SCHD·BITO·VCX 134건, 이어서 QQQ·UPRO·SOXX·SSO·O·NVO·ETHU·SOXL·USD·AAPL 154건. 계좌4 거래는 481건이 됐다.
    ...
    - 현금잔고는 2026-05-11부터 음수(현재 −2,660만). 그날 id 6으로 3,306만을 이체했는데 직전 잔고가 +79만뿐이었다 — 그 이체 하나가 원인이고 나머지 구간은 앞뒤가 맞는다.
