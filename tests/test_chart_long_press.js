...
ctx.bindChartLongPress(target, () => {visible = true;}, () => {visible = false;}, 0);
fire('pointerdown');
assert.equal(visible, true, 'trade marker displays immediately without advancing the clock');
assert.equal(timers.size, 0, 'immediate marker does not start a hold timer');
fire('pointerup'); fire('pointerleave');
assert.equal(visible, true, 'marker stays visible after release');
outsideTouch();
assert.equal(visible, false, 'another touch dismisses marker details');
assert.equal(dismissListeners.size, 0);
fire('pointerdown'); fire('pointercancel');
assert.equal(visible, false, 'cancelled marker touch clears details');
assert.match(source, /bindChartLongPress\(marker,[\s\S]*?classList\.add\("hidden"\), 0\)/);
