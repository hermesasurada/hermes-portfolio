...
const desktopCss = read('styles.css').slice(0, read('styles.css').lastIndexOf('@media (max-width: 980px)'));
assert.match(desktopCss, /\.transaction-actions \{[^}]*flex: 0 0 auto;[^}]*flex-wrap: nowrap;/);
assert.match(desktopCss, /\.transaction-panel > \.toolbar \.title-tools \{[^}]*width: auto;[^}]*min-width: 0;[^}]*flex: 1 1 0;/);
